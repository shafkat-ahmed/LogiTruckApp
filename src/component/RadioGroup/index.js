...
export default class RadioGroup extends Component {
    ...
    validate(data) {
        ...
        data.map(e => {
            e.color = e.color ? e.color : 'rgba(79, 186, 111, 1)'
    ...
    render() {
        ...
        return (
            <TouchableOpacity
                ...
                <View style={[styles.border, { borderColor: data.color, width: data.size, height: data.size, borderRadius: data.size / 2, alignSelf: 'center' },]}>
                    {data.selected &&
                        <View style={{ backgroundColor: data.color, width: data.size / 2, height: data.size / 2, borderRadius: data.size / 2, }}
                        />}
                </View>
                {
